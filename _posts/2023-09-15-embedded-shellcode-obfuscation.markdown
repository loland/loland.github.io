...
Note that results of this research project are time-sensitive. Vendor flags on fresh malware samples will increase drastically with time, likely due to backend sandboxes updating AV signatures. These updated AV signatures, I suspect, are based on fuzzy-hashes. Because while the same code compiled twice produces different hash values, detection rate coincides. Results published in this blog will differ based on time of reading.
...
<br>
### 6. Continuation
Continuation of this research in "Embedded Shellcode Obfuscation Part 2".